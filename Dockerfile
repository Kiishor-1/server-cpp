...
# Install OpenSSL (for JWT-CPP)
RUN apt-get install -y openssl

# Install JWT-CPP dependencies
RUN git clone https://github.com/Thalhammer/jwt-cpp /tmp/jwt-cpp && \
    cd /tmp/jwt-cpp && \
    mkdir build && \
    cd build && \
    cmake -Djwt-cpp_BUILD_EXAMPLES=OFF .. && \
    make && \
    make install
...
# Copy the JWT-CPP and OpenSSL libraries
COPY --from=build-stage /usr/local/lib/libjwt-cpp.so* /usr/local/lib/
COPY --from=build-stage /usr/lib/x86_64-linux-gnu/libssl.so* /usr/lib/
COPY --from=build-stage /usr/lib/x86_64-linux-gnu/libcrypto.so* /usr/lib/

# Copy the cURL shared libraries
COPY --from=build-stage /usr/lib/x86_64-linux-gnu/libcurl.so* /usr/lib/
...
# # Copy the JWT-CPP and OpenSSL libraries
# COPY --from=build-stage /usr/local/lib/libjwt-cpp.so* /usr/local/lib/
# COPY --from=build-stage /usr/local/ssl/lib/ /usr/local/ssl/lib/ 
