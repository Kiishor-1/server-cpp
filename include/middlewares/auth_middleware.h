...
//         const std::vector<std::string> protectedRoutes = {"/protected", "/user/profile"};

//         // Check if the current request path is in the list of protected routes
//         if (std::find(protectedRoutes.begin(), protectedRoutes.end(), req.url) != protectedRoutes.end()) {
//             const auto token = req.get_header_value("Authorization");
//             if (token.empty() || !validate_token(token)) {
//                 res.code = 401;  // Unauthorized
//                 res.write("Unauthorized: Invalid or missing token");
//                 res.end();
//                 return;
//             }
//         }
...
//     {
//         // List of routes that require authentication
//         const std::vector<std::string> protectedRoutes = {"/create", "/update", "/delete","/listings/<string>/reviews","/listings/<string>/reviews/<string>"};

//         // Check if the current request path starts with one of the protected routes
//         for (const auto &route : protectedRoutes)
//         {
//             if (req.url.find(route) == 0) // Check if the request URL starts with the protected route
//             {
...
//                 std::cout << "Using secret key: " << get_secret_key() << std::endl;
...
//                 ctx.user_id = userId;
//                 std::cout << "User ID stored in context: " << ctx.user_id << std::endl; // Log the stored user ID

//                 // If authorization is successful, we can break out of the loop.
//                 break;
//             }
//         }
//         // Allow the request to proceed if it's not a protected route
...
//         {
//             auto decoded = jwt::decode(token);
//             std::cout << "Decoded token payload: " << decoded.get_payload() << std::endl;
//             auto verifier = jwt::verify()
//                                 .allow_algorithm(jwt::algorithm::hs256{get_secret_key()})
//                                 .with_issuer("auth-server");
//             verifier.verify(decoded);

//             // Extract user_id from the token's payload (assuming it's stored in the "sub" claim)
