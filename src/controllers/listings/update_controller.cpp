...
//         // Deserialize BSON to crow::json::wvalue for modification
//         auto current_listing_json = crow::json::load(bsoncxx::to_json(listing_view));  // No `.wvalue()` here, just load the JSON
//         std::string listing_json_str = bsoncxx::to_json(listing_view);

//         // Convert JSON string into a mutable crow::json::wvalue (which is modifiable)
//         crow::json::wvalue current_listing_json = crow::json::load(listing_json_str);

//         // Update fields provided in the request body
//         if (json_body.has("country"))
//             current_listing_json["country"] = json_body["country"];
//         if (json_body.has("location"))
//             current_listing_json["location"] = json_body["location"];
//         if (json_body.has("price"))
//             current_listing_json["price"] = json_body["price"];
//         if (json_body.has("title"))
//             current_listing_json["title"] = json_body["title"];
//         if (json_body.has("description"))
//             current_listing_json["description"] = json_body["description"];
...
//                 return;
//             }
//             current_listing_json["category"] = category;
//         }

//         if (json_body.has("image"))
//         {
//             current_listing_json["image"]["url"] = json_body["image"]["url"];
//             current_listing_json["image"]["filename"] = json_body["image"]["filename"];
//         }
//         if (json_body.has("geometry"))
//         {
//             current_listing_json["geometry"]["coordinates"][0] = json_body["geometry"]["coordinates"][0];
//             current_listing_json["geometry"]["coordinates"][1] = json_body["geometry"]["coordinates"][1];
//         }

//         crow::json::rvalue current_listing_rvalue = current_listing_json;

//         // Convert the updated JSON to Listing object
//         Listing current_listing = Listing::fromJson(current_listing_rvalue);

//         // Validate updated listing
//         current_listing.validate();

//         // Convert the updated JSON back to BSON
//         auto updated_bson = bsoncxx::from_json(crow::json::dump(current_listing_json));

//         // Perform the update operation in MongoDB
//         bsoncxx::builder::stream::document update_doc{};
//         update_doc << "$set" << updated_bson;
