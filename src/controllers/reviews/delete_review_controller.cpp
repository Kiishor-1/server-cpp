...
// void handleDeleteReview(const crow::request &req, crow::response &res, MongoClient &mongoClient, const std::string &reviewId, const std::string &userId)
// {
//     auto client = mongoClient.createClient();
//     auto reviewCollection = client["wanderlust2"]["reviews"];
//     auto listingCollection = client["wanderlust2"]["listings"];
...
//         // Get the listing ID from the review
//         std::string listingId = review->view()["listing"].get_oid().value.to_string();
