...
//         // Build the JSON response (excluding the password)
//         crow::json::wvalue userResponse;
//         userResponse["username"] = userDoc["username"].get_utf8().value.to_string();
//         userResponse["_id"] = userDoc["_id"].get_oid().value.to_string();

//         if (userDoc.find("firstName") != userDoc.end()) {
//             userResponse["firstName"] = userDoc["firstName"].get_utf8().value.to_string();
//         }
//         if (userDoc.find("lastName") != userDoc.end()) {
//             userResponse["lastName"] = userDoc["lastName"].get_utf8().value.to_string();
//         }
//         if (userDoc.find("bio") != userDoc.end()) {
//             userResponse["bio"] = userDoc["bio"].get_utf8().value.to_string();
//         }

//         // Fetch bookings (assuming bookings are stored in a 'bookings' collection linked by userId)
//         bsoncxx::builder::basic::document bookings_query{};
//         bookings_query.append(bsoncxx::builder::basic::kvp("userId", bsoncxx::oid(user_id)));

//         auto maybe_bookings = bookingsCollection.find_one(bookings_query.view());

//         if (maybe_bookings) {
//             userResponse["bookings"] = crow::json::load(bsoncxx::to_json(maybe_bookings->view()));
//         } else {
//             userResponse["bookings"] = "No bookings found"; // Handle case if no bookings are available
//         }

//         // Send response
//         res.set_header("Content-Type", "application/json");
//         res.write(userResponse.serialize());
//         res.end();
...
//         for (auto&& doc : cursor) {
//             bookingsArray.push_back(crow::json::load(bsoncxx::to_json(doc)));
//         }
